...
session_start(); 
include 'db.php'; 
$sql = "SELECT * FROM products WHERE category = 'Cupcake'";
...
<head>
    ...
    <title>Fresh Cupcakes - Serandib Twist</title>
    <style>
        .navbar-nav .dropdown-menu {
            display: block !important; opacity: 0; visibility: hidden;
            transform: translateY(-30px) scaleY(0); transform-origin: top;
            transition: all 0.1s cubic-bezier(0.23, 1, 0.32, 1); 
            border: none; border-top: 3px solid #f1c40f; 
            box-shadow: 0 15px 35px rgba(0,0,0,0.2); border-radius: 0 0 10px 10px;
        }
        .nav-item.dropdown:hover .dropdown-menu { opacity: 1; visibility: visible; transform: translateY(0) scaleY(1); }
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      ...
      <div class="d-flex align-items-center gap-3">
        <form class="d-flex" action="search.php" method="GET">
          <input class="form-control me-2 rounded-pill" name="query" type="search" placeholder="Search sweets..." required>
...
<section class="py-5" style="background-color: #fdfaf5;">
  <div class="container">
    <h1 class="text-center mb-2" style="color: #3d2b1f; font-family: 'Playfair Display', serif;">Serandib Twist Cupcakes</h1>
    <p class="text-center mb-5 text-muted">Freshly Baked & 100% Natural</p>
    
    <div class="row g-4 justify-content-center">
      <?php if ($result && $result->num_rows > 0) { while($row = $result->fetch_assoc()) { ?>
          <div class="col-md-4 mb-4">
              <div class="card h-100 border-0 shadow-sm text-center p-3">
                  <img src="images/<?php echo $row['image']; ?>" class="card-img-top mx-auto" alt="<?php echo $row['name']; ?>">
                  <div class="card-body d-flex flex-column">
                      ...
                      <form action="cart.php" method="POST">
                          ...
                          <div class="d-grid gap-2 mt-auto">
                              <button type="submit" name="add_to_cart" class="btn btn-dark rounded-pill py-2 fw-bold">Add to Cart</button>
                              <a href="product_details.php?id=<?php echo $row['id']; ?>" class="btn btn-outline-dark rounded-pill py-2 small">Explore Sweet</a>
                          </div>
                      </form>
                  </div>
              </div>
